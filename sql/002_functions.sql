...
-- ---------------------------------------------------------------------
-- admin_delete_coach(...) — service role only. Refuses to delete a coach
-- who has ever marked attendance, ticks, checkpoints, feedback, or notes —
-- that history belongs to the students it's attached to, not the coach,
-- so it must not vanish because the coach account is removed. Deactivate
-- is the path for a coach with any activity; this is only for an account
-- that was never used.
-- ---------------------------------------------------------------------
